...
async function loadZones() {
  try {
    ...
    if (Array.isArray(zones)) {
      ...
      console.log(
        `📍 Found ${zones.length} total zones, showing ${activeZones.length} active database zones`,
      );
...
function addZoneMarkers(zones) {
  ...
  zones.forEach((zone, index) => {
    ...
    const marker = L.marker(position).bindPopup(buildPopupContent(zone));
