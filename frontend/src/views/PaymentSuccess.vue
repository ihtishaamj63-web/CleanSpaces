    ...
    <div class="auth-card">
      ...
      >
        ...
        <p class="confirm-text">
          Thank you! Your contribution to your zone has been received.
        </p>

        <div class="details">
          <div class="row">
            <span>Reference</span
            ><span>#CS-{{ String(payment.id).padStart(5, "0") }}</span>
          </div>
          <div class="row">
            <span>Amount</span><span>R{{ payment.amount }}</span>
          </div>
          <div class="row">
            <span>Method</span><span>{{ methodLabel }}</span>
          </div>
        </div>
      ...
      <!-- FAILED -->
      <div
        v-else-if="payment && payment.status === 'failed'"
        class="confirmation"
      >
        <div class="check failed">✕</div>
        <h2 class="confirm-title">Payment Not Completed</h2>
        <p class="confirm-text">
          The payment was cancelled or declined — no money has left your
          account.
        </p>
      ...
      <div v-else-if="loadFailed" class="loading-state">
        ...
        <router-link to="/resident/dashboard" class="submit-btn"
          >Go to My Dashboard →</router-link
        >
      </div>
...
async function load() {
  try {
    ...
    if (payment.value?.status === "pending" && retries++ < 4)
...
.submit-btn:hover {
  ...
  box-shadow: 0 10px 26px rgba(23, 107, 58, 0.28);
}
