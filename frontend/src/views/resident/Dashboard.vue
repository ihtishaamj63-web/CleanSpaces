    ...
    <section class="dash-hero">
      <div class="hero-content">
        ...
        <p>
          {{
            data.hasZone
              ? data.zone.neighborhood +
                " · " +
                data.zone.households +
                " households pooling their cleanups"
    ...
    <main class="dash-main">
      ...
      <template v-else>
        ...
        <section v-if="!data.hasZone" class="get-started card">
          ...
          <div class="cta-row">
            <router-link to="/payment" class="primary-btn"
              >Join &amp; pay for my zone</router-link
            >
            <router-link to="/how-it-works" class="secondary-btn"
              >Register a new zone</router-link
            >
          </div>
        </section>
          ...
          <section class="zone-grid">
            <div class="card activation-card">
              ...
              <div class="activation-figures">
                ...
                <span class="activation-of"
                  >of {{ data.zone.threshold }} households paid this month</span
                >
                <span class="activation-remaining"
                  >{{ remaining }} to go until your zone activates</span
                >
              </div>
              ...
              <div class="status-banner" :class="data.zone.myStatus">
                <p class="status-msg">
                  <template v-if="data.zone.myStatus === 'paid'">
                    ● You're covered this month — your contribution keeps the
                    weekly cleanups running. Next payment due {{ nextDue }}.
                  </template>
                  <template v-else>
                    Your payment is due. R{{ perMonth }} secures this month's
                    cleanup for your zone.
                  </template>
                </p>
                <router-link
                  v-if="data.zone.myStatus !== 'paid'"
                  to="/payment"
            ...
            <div v-else class="empty-note card">
              <div class="empty-icon" aria-hidden="true">
                <svg viewBox="0 0 24 24" fill="currentColor">
                  <path
                    d="M12 3l1.9 5.6 5.6 1.9-5.6 1.9L12 18l-1.9-5.6-5.6-1.9 5.6-1.9z"
                  />
                  <path
                    d="M19 15l.9 2.6 2.6.9-2.6.9-.9 2.6-.9-2.6-2.6-.9 2.6-.9z"
                    opacity="0.55"
                  />
                </svg>
              </div>
              ...
              <p>
                Once your zone activates, photo proof of every cleanup lands
                here.
              </p>
            </div>
          </section>
            ...
            <div class="card ledger-card">
              <template v-if="payments.length">
                <div v-for="p in payments" :key="p.id" class="ledger-row">
                  ...
                  >
                  <span class="ledger-status" :class="p.status">{{
                    p.status
                  }}</span>
              ...
              <p v-else class="muted-note">
                No payments yet.
                <router-link to="/payment" class="text-link"
                  >Make your first →</router-link
                >
              </p>
            </div>
          </section>
        ...
        <section class="report-section">
          <div class="report-intro">
            ...
            <div class="form-note">
              <span>✓</span>
              <p>
                Every report is reviewed and scheduled by our crews — you'll see
                the status of each one right here.
              </p>
            </div>
          </div>
          ...
          <form class="report-form" @submit.prevent="submitRequest">
            <p
              v-if="requestMessage"
              :class="['request-msg', requestError ? 'bad' : 'good']"
            >
              {{ requestMessage }}
            </p>
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <input
                  id="location"
                  v-model.trim="requestForm.location_name"
                  required
                  placeholder="e.g. NY108 corner"
                />
              </div>
              <div class="form-group">
                ...
                <input
                  id="suburb"
                  v-model.trim="requestForm.suburb"
                  required
                  placeholder="e.g. Manenberg"
                />
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <input
                ...
                required
                placeholder="e.g. NY108 & Eisleben Road"
              />
            </div>
            ...
            <div class="form-group">
              ...
              <textarea
                ...
                required
                rows="3"
                placeholder="Describe the waste, the size of the dump, how long it's been there…"
              ></textarea>
            </div>
            ...
            <div class="form-group">
              ...
              <span class="field-hint"
                >On your phone this offers the camera or your gallery — on
                desktop, a file picker.</span
              >

              <div v-if="photoPreview" class="photo-preview">
                <img :src="photoPreview" alt="Photo preview" />
                <button type="button" class="photo-remove" @click="clearPhoto">
                  Remove photo
                </button>
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              />
              <span class="field-hint"
                >When should the team ideally get to it? Leave empty if
                anytime.</span
              >
            </div>

            <button type="submit" class="submit-btn" :disabled="requestSaving">
              <span v-if="!requestSaving"
                >Send to CleanSpaces <span class="submit-arrow">→</span></span
              >
          ...
          <div class="request-grid">
            ...
            >
              <div class="request-top">
                ...
                <span class="request-status" :class="r.status">{{
                  statusLabel(r.status)
                }}</span>
              </div>
              <p class="request-loc">{{ r.address }}, {{ r.suburb }}</p>
              <p class="request-desc" v-if="r.description">
                {{ r.description }}
              </p>
              <img
                v-if="r.photo_url"
              ...
              <small>
                ...
                <template v-if="r.preferred_date">
                  · Preferred {{ formatDate(r.preferred_date) }}</template
                >
              </small>
            </article>
          </div>
...
async function submitRequest() {
  ...
  try {
    ...
    if (requestForm.preferred_date)
      body.append("preferred_date", requestForm.preferred_date);
    ...
    requestError.value = true;
    requestMessage.value =
      e.response?.data?.message || "Unable to submit your request.";
  } finally {
...
async function load() {
  ...
  try {
    ...
    await nextTick();
    setTimeout(() => {
      progressDisplay.value = progressPct.value;
    }, 300);
    if (data.value.hasZone) {
...
.status-banner.paid .status-msg {
  color: #24633a;
}
.status-banner.pending .status-msg {
  color: #7a5300;
  font-weight: 600;
}
...
.tag.before {
  left: 18px;
  background: rgba(23, 37, 26, 0.65);
  color: #fff;
}
.tag.after {
  right: 18px;
  background: rgba(23, 107, 58, 0.85);
  color: #fff;
}
