...
const router = createRouter({
  ...
  routes: [
    ...
    // Resident-only flow
    { path: '/payment', name: 'payment', component: () => import('../views/Payment.vue'), meta: { requiresAuth: true, requiresResident: true } },
    { path: '/payment/success/:id', name: 'paymentSuccess', component: () => import('../views/PaymentSuccess.vue'), meta: { requiresAuth: true, requiresResident: true } },
    { path: '/resident', redirect: '/resident/dashboard' },
    { path: '/resident/dashboard', name: 'residentDashboard', component: () => import('../views/resident/Dashboard.vue'), meta: { requiresAuth: true, requiresResident: true } },
    ...
    { path: '/admin/reviews', name: 'adminReviews', component: () => import('../views/admin/Reviews.vue'), meta: { requiresAuth: true, requiresAdmin: true } },
    ...
    // Everything else falls back to home
    { path: '/:pathMatch(.*)*', redirect: '/' }
  ]
})

/* Route guards, in order:
   1. requiresAuth     — any logged-in user (token present)
   2. requiresAdmin    — admins only; everyone else is sent home
   3. requiresResident — residents only; admins are sent to their own panel */
router.beforeEach((to) => {
  ...
  if (to.meta.requiresResident && role !== 'resident') return { name: 'adminDashboard' }
})
